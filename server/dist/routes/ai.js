...
router.post('/generate', async (req, res) => {
    try {
        ...
        if (!firstUser)
            return res.json({ text: "Error: No user found. Please register first." });
        ...
        if (lowerPrompt.includes('add client') || lowerPrompt.includes('create client') || lowerPrompt.includes('new client')) {
            ...
            let client = await db_1.prisma.client.findFirst({ where: { email, userId: firstUser.id } });
            if (!client) {
        ...
        try {
            ...
            if (data.action === 'create_invoice') {
                ...
                let client = await db_1.prisma.client.findFirst({ where: { name: clientName, userId: firstUser.id } });
                if (!client) {
                    client = await db_1.prisma.client.create({
